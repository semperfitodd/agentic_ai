...
function buildSprintReportMarkdown(data, meta) {
    const lines = [
...
const handler = async (event) => {
    ...
    const loadedAnalyses = await Promise.all(event.analyses.map(ref => loadAnalysisFromS3(bucketName, ref.owner, ref.repo, ref.prNumber)));
    const validAnalyses = loadedAnalyses.filter((analysis) => analysis !== null && analysis.metadata !== undefined);
