...
function buildAnalysisPrompt(details, readme) {
...
const handler = async (event) => {
    ...
    shared_1.logger.info('Analyzing PR', { owner, repo, prNumber });
    const bucketName = event.s3Bucket ?? (0, shared_1.requireEnv)('RESULTS_BUCKET');
    try {
        if (!owner || !repo || !prNumber)
            throw new Error('owner, repo, and prNumber are required');
        let readme = '';
        if (event.readmeS3Key) {
            const readmeMap = await (0, shared_1.getJson)(bucketName, event.readmeS3Key);
            readme = readmeMap[`${owner}/${repo}`] ?? '';
            shared_1.logger.info('Loaded README map', { owner, repo, bytes: readme.length });
        }
        const s3Prefix = `pr-details/${owner}/${repo}/${prNumber}/`;
        const detailsKey = await (0, shared_1.listLatestKey)(bucketName, s3Prefix);
        if (!detailsKey)
            throw new Error(`No PR details found in S3 at prefix: ${s3Prefix}`);
        const prDetails = await (0, shared_1.getJson)(bucketName, detailsKey);
        if (!prDetails.pr)
            throw new Error('PR data is missing from S3');
        const prompt = buildAnalysisPrompt({ ...prDetails, owner, repo }, readme);
        const analysis = await (0, shared_1.invokeClaude)({
            user: prompt,
        ...
        await (0, shared_1.putJson)(bucketName, analysisS3Key, fullAnalysis, {
            owner,
            repo,
            prNumber: prDetails.pr.number.toString(),
        });
        shared_1.logger.info('Analysis stored', { owner, repo, prNumber, key: analysisS3Key, bytes: Buffer.byteLength(analysis) });
        return (0, shared_1.stepResponse)(200, { owner, repo, prNumber: prDetails.pr.number });
    }
    catch (error) {
        shared_1.logger.error('Error analyzing PR', { owner, repo, prNumber, message: error instanceof Error ? error.message : String(error) });
        return (0, shared_1.stepResponse)(500, { error: error instanceof Error ? error.message : 'Failed to analyze PR', owner, repo, prNumber });
    }
};
